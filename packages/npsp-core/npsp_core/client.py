"""Framework-neutral Salesforce NPSP client.

Fetches donor Contact records, Opportunity gift histories, and NPSP
engagement metrics, returning plain ``{"text", "metadata"}`` donor records.

This module holds ALL the Salesforce logic. The Haystack component and the
MCP server are thin adapters over it, so the SOQL lives in exactly one place.
Ported verbatim from the LlamaIndex ``SalesforceNPSPReader`` — only the final
output type changed (dict instead of a framework Document).
"""
...
# A framework-neutral donor record: {"text": <narrative>, "metadata": <fields>}
DonorRecord = Dict[str, Any]


class SalesforceNPSPClient:
    """Client for Salesforce Nonprofit Success Pack (NPSP) donor data.
    ...
    Args:
        ...
        affinity_score_fn: Optional callable (metadata_dict) -> float for
            injecting ML-derived affinity scores at load time.
    """
...
{gift_history_text}""".strip()
    ...
    def load_donors(
        ...
        """Load donor records from Salesforce NPSP.

        Args:
            contact_ids: Specific Contact IDs to load. If provided,
                soql_filter and limit are ignored.
            soql_filter: SOQL WHERE clause to filter Contact records.
            limit: Maximum number of donor records to return.

        Returns:
            One ``{"text", "metadata"}`` record per donor Contact.
        """
        ...
        return [self._build_record(c, opp_map) for c in contacts]
